...
public class mod_ComputerCraft extends BaseModMp {
    ...
    public static void openDiskDriveGUI(EntityPlayer entityplayer, TileEntityDiskDrive drive) {
        ContainerDiskDrive container = new ContainerDiskDrive((IInventory)entityplayer.inventory, drive);
        ModLoader.OpenGUI((EntityPlayer)entityplayer, (int)diskDriveGUIID, (IInventory)entityplayer.inventory, (Container)container);
    }

    public static void sendToPlayer(EntityPlayer player, Packet230ModLoader packet) {
        if (player instanceof EntityPlayerMP) {
            ModLoaderMp.SendPacketTo((BaseModMp)instance, (EntityPlayerMP)((EntityPlayerMP)player), (Packet230ModLoader)packet);
        }
    }
    ...
    public void HandlePacket(Packet230ModLoader packet, EntityPlayerMP player) {
        World world = ModLoaderMp.GetPlayerWorld((EntityPlayer)player);
        if (world != null) {
            if (packet.packetType == 11) {
                for (int n = 0; n < packet.dataInt.length; ++n) {
                    int id = packet.dataInt[n];
                    ItemDisk.sendDiskLabelToPlayer((EntityPlayer)player, id);
                }
            } else {
